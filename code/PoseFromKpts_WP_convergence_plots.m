function output = PoseFromKpts_WP_convergence_plots(W,dict,varargin)
...
for iter = 1:1000
    ...
    %Debug operations
    iters = [iters, iter];
    PrimResVect = [PrimResVect, PrimRes];
    DualResVect = [DualResVect, DualRes];
...
%Added debug prints:
disp('Weak perspective iterations:')
disp(iter)
figure
subplot(2,2,1);
plot(iters, PrimResVect, iters, DualResVect)
title('Weak Perspective Residuals')
grid on
legend('PrimRes','DualRes')
subplot(2,2,2);
semilogy(iters, PrimResVect, iters, DualResVect)
title('Weak Perspective Residuals, log scale')
grid on
legend('PrimRes','DualRes')
