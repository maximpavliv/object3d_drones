function output = PoseFromKpts_FP_convergence_plots(W,dict,varargin)
...
for iter = 1:1000
    ...
    %Debug operations
    iters = [iters, iter];
    ResVectAbs = [ResVectAbs, abs(fval-fvaltm1)/(fvaltm1+eps)];
%    ResVectNoabs = [ResVectNoabs, (fval-fvaltm1)/(fvaltm1+eps)];
...
%figure
subplot(2,2,3);
%plot(iters, ResVectAbs, iters, ResVectNoabs)
plot(iters, ResVectAbs)
title('Full Perspective Residuals')
grid on
%legend('Res Abs', 'Res No abs')
legend('Res')
subplot(2,2,4)
%semilogy(iters, ResVectAbs, iters, ResVectNoabs)
semilogy(iters, ResVectAbs)
