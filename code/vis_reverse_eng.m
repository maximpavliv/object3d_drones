function vis_reverse_eng(img, heatmap, center, scale)
    ...
    imshow(img_crop); hold on; %hold on?
    ...
    % visualization each heatmap
    nplot_heatmap = size(heatmap, 3);
    h = figure('position',[100,100,nplot_heatmap*150,150]);
            
    % heatmap
    for i = 0:nplot_heatmap-1
        subplot('position',[i/nplot_heatmap 0 1/nplot_heatmap 1]);
        response = heatmap(:,:,i+1);
        max_value = max(max(response));
        mapIm = imresize(mat2im(response, jet(100), [0 max_value]),[200,200],'nearest');
        imToShow = mapIm*0.5 + (single(img_crop)/255)*0.5;
        imagesc(imToShow); axis equal off
